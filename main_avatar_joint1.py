    ...
    def test(self):
        ...
        for idx in tqdm(range(data_num), desc = 'Rendering avatars...'):
            ...
            if self.opt['test'].get('render_skeleton', False):
                ...
                if self.opt['test'].get('save_skeleton', False):
                    os.makedirs(output_dir + '/skeleton_ply', exist_ok=True)
                    skel_mesh.export(
                        output_dir + '/skeleton_ply/%08d.ply' % item['data_idx'])
            ...
            # ADD THIS SECTION HERE - Save joints data
            if self.opt['test'].get('save_joints', True):  # Enable by default, can be disabled in config
                os.makedirs(output_dir + '/joints', exist_ok = True)

                # Get joints data
                joints_data = item['joints'].cpu().numpy()  # Shape: (num_joints, 3)

                # Save as numpy file (.npy) - most efficient for numerical data
                np.save(output_dir + '/joints/%08d.npy' % item['data_idx'], joints_data)
