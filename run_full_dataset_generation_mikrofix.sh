...
# --- bash-only guard (avoid cryptic syntax errors if run with sh) ---
if [ -z "${BASH_VERSION:-}" ]; then
  echo "ERROR: This script must be run with bash. Use: bash $0 <SUBJECT_ID>" 1>&2
  exit 2
fi
...
# Paths (可用环境变量覆盖)
# -----------------------
BASE_DIR="${BASE_DIR:-/home/fzhi/fzt}"

ANIM_ROOT="${ANIM_ROOT:-${BASE_DIR}/AnimatableGaussians}"
HUGS_ROOT="${HUGS_ROOT:-${BASE_DIR}/ml-hugs}"

POSE_DIR="${POSE_DIR:-${ANIM_ROOT}/data_body/pose/AMASS/CMU/CMU/omilabpose}"
...
# 输出根目录
HDD_BASE="${HDD_BASE:-/mnt/data2/fzhi}"
...
SCENE_PT_ROOT="${SCENE_PT_ROOT:-${BASE_DIR}/scene/scenefixed}"
...
# 兼容 MAX_PARALLEL 为空/非数字/<=0 的情况（默认当作 1）
_sanitize_max_parallel() {
...
wait_all_bg() {
  # BG_PIDS 为空时不要 wait（原来用默认值展开会在空数组时产生空元素）
  ((${#BG_PIDS[@]})) || return 0
