...
def build_opengl_projection(fovx, fovy, znear=0.01, zfar=100.0, device="cuda"):
    # OpenGL-style projection (right-handed, camera looks down -Z)
    tan_half_fovx = math.tan(fovx / 2.0)
    tan_half_fovy = math.tan(fovy / 2.0)

    P = torch.zeros((4, 4), dtype=torch.float32, device=device)
    P[0, 0] = 1.0 / tan_half_fovx
    P[1, 1] = 1.0 / tan_half_fovy
...
def load_camera_data(camera_json_path, frame_idx=0):
    ...
    H = int(cam_j['image_height'][i])
    Wimg = int(cam_j['image_width'][i])

    # 1) 读取 view
    WV = torch.tensor(cam_j['world_view_transform'][i], dtype=torch.float32, device='cuda')
    WV = WV.view(4, 4)

    # 2) （可选）如果你需要做坐标系对齐，就在这里统一改 WV
    # 例如：COLMAP(CV) -> OpenGL
    # fix = torch.eye(4, device='cuda')
    # fix[1, 1] = -1
    # fix[2, 2] = -1
    # WV = fix @ WV

    # 3) 现场重算 projection 和 full_proj
    # 注意：near/far 如果 json 里有就用，没有就给个合理默认
    znear = float(cam_j['near'][i]) if 'near' in cam_j and len(cam_j['near']) > i else 0.01
    zfar = float(cam_j['far'][i]) if 'far' in cam_j and len(cam_j['far']) > i else 100.0

    P = build_opengl_projection(fovx, fovy, znear=znear, zfar=zfar, device='cuda')
    full_proj = WV @ P

    # 4) camera_center：从 WV 反推（别信 json 里那个）
    # WV 是 world->view：其逆的平移部分就是相机中心
    WV_inv = torch.inverse(WV)
    cam_center = WV_inv[:3, 3]
